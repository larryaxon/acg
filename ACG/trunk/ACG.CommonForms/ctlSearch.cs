...
namespace ACG.CommonForms
{
  public partial class ctlSearch : UserControl
  {
    ...
    public ISearchDataSource SearchExec
    {
      get { return searchExec; }
      set { searchExec = value; }
    }
    //public ISearchDataSource SearchExec = null;

    private string id_DescSplitter = ":";
    ...
    public string ID_DescSplitter
    {
      get { return id_DescSplitter; }
      set
      {
        if (string.IsNullOrEmpty(value))
          id_DescSplitter = ":";
        else
          id_DescSplitter = value;
      }
    }
    ...
    private void txtSearch_Leave(object sender, EventArgs e)
    {
      if (string.IsNullOrEmpty(txtSearch.Text))
      {
        _id = string.Empty;
        _Name = string.Empty;
        //lmv66 2014 0104 Begin 
        lstSearchList.SelectedIndex = -1;
        return;
        //lmv66 2014 0104 End 
      }
      else if (AddNewMode)
    ...
    private void btnExpand_Click(object sender, EventArgs e)
    {
       ...
       */
      if (!_navigatingFromList || txtSearch.Text == String.Empty)  
        expandList(false);
      _navigatingFromList = false;
      if (Collapsed)
        txtSearch.Focus();
    ...
    private bool search(string criteria, bool autoSelect, bool forceChangeFocus)
    {
      ...
      else
      {
        ...
        foreach (KeyValuePair<string, string> val in AdditionalValues)
        {
          ...
          {
            //"{0,-7}{2}{1}"
            if (_fixKeySpace.Equals("-1"))
              newItem = string.Format("{0}{2} {1}", val.Key, val.Value, id_DescSplitter);
            else
              newItem = string.Format("{0,-" + _fixKeySpace + "}{2} {1}", val.Key, val.Value, id_DescSplitter);
          }
    ...
    private string getFromID(string id)
    {
      ...
      { 
        ...
        //"{0,-7}: {1}"
        if (_fixKeySpace.Equals("-1"))
          return string.Format("{0}{2} {1}", id, AdditionalValues[id], id_DescSplitter);
        else
          return string.Format("{0, -" + _fixKeySpace + "}{2} {1}", id, AdditionalValues[id], id_DescSplitter);
      }
    ...
    private void setIDandDescription(string s)
    {
      ...
      {
        if (s.Contains(id_DescSplitter)) // the string already has both id and desc
        {
          //string[] parts = s.Split(new char[] { ':' });
          string[] parts = s.Split(new string[] { id_DescSplitter }, StringSplitOptions.None);
          if (parts == null)
            _id = _Name = s;
          else
          {
      ...
      if (s.Contains(id_DescSplitter))// we have a delimiter
      {
        //string[] parts = s.Split(new char[] { ':' });
        string[] parts = s.Split(new string[] { id_DescSplitter }, StringSplitOptions.None);
        if (parts == null)
    ...
    private string getDescription(string entry)
    {
      if (string.IsNullOrEmpty(entry))
        return string.Empty;
      int pos = entry.IndexOf(id_DescSplitter);
      if (pos <= 0)
        return entry;
