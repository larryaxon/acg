...
namespace ACG.Sys.Data
{
  public partial class DataSource
  {
    public ACGTable getTimeData(int timeID)
    {
      ...
      return ACGCommonFunctions.convertDataSetToCCITable(getDataFromSQL(sql));
    }
...
case when ProjectID = 'All' then 1 else 0 end";
    ...
    public DataSet getTimeSummary(string resourceid, string customerid, 
      ...
      DateTime startDate, DateTime endDate, ACGCommonData.TimeEntrySummaryMode mode, bool includeSubProject,
      bool includeOnlyUnpaidTime)
    {
      ...
      switch (mode)
      {
        case ACGCommonData.TimeEntrySummaryMode.Daily:
    ...
    {1} group by {0} timeworkeddate order by timeworkeddate";
          break;
        case ACGCommonData.TimeEntrySummaryMode.Weekly:
    ...
    order by datepart(year, timeworkeddate), datepart(week, timeworkeddate) ";
          break;
        case ACGCommonData.TimeEntrySummaryMode.ByProject:
    ...
    {
      string sql = string.Empty;
      int id = fields.ContainsKey(ACGCommonData.fieldTIMEID) ? CommonFunctions.CInt(fields[ACGCommonData.fieldTIMEID], -1) : -1;
      ...
      if (id == -1) // new record
      {
        ...
        if (!fields.ContainsKey(ACGCommonData.fieldCREATEDDATETIME.ToLower()))
        {
          fieldList.Append(", ");
          fieldList.Append(ACGCommonData.fieldCREATEDDATETIME);
          valueList.Append(", '");
          valueList.Append(DateTime.Now.ToString(CommonData.FORMATLONGDATETIME));
          valueList.Append("'");
        }
