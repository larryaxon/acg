...
namespace ACG.Common
{
    public class Mailer
    {
        ...
        private static void LoadConfigInfo()
        {
            ...
            cc = System.Configuration.ConfigurationManager.AppSettings["smtpcc"];
        }
        ...
        public static string SendMail(string toEmail, string subject, string content, string attachment, bool isHTML)
        {
          try
          {
            ...
            {
              string[] copyEmailList = cc.Split(';');
              if (copyEmailList.Length > 0)
              {
                foreach (string email in copyEmailList)
                {
                  mess.CC.Add(new MailAddress(email));
                }
              }
            }
