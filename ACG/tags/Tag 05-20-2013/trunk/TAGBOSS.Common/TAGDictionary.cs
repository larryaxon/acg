...
namespace TAGBOSS.Common
{
  ...
  public class TAGDictionary
  {
    ...
    Dictionary<string, Dictionary<string, Dictionary<string, object>>> doDictionary = new Dictionary<string, Dictionary<string, Dictionary<string, object>>>(StringComparer.CurrentCultureIgnoreCase);
    ...
    /// for each attribute
    /// </summary>
    public TAGDictionary(EntityAttributesCollection eacDictionary)
    {
      load(eacDictionary);
    }
    ...
    {
      return doDictionary.ContainsKey(cATTRIBUTE) && doDictionary[cATTRIBUTE].ContainsKey(attributeName);
    }
    ...
    {
      return (doDictionary.ContainsKey(dictionaryType) &&
              doDictionary[dictionaryType].ContainsKey(dictionaryItem) &&
              doDictionary[cDICTIONARY][dictionaryType].ContainsKey(propertyName));
    }
    ///// <summary>
    ...
    public ArrayList ValidItems(string itemType)
    {
      ...
      if (doDictionary.ContainsKey(itemType))
      {
        foreach (KeyValuePair<string, Dictionary<string, object>> entry in doDictionary[itemType])
    ...
    public ArrayList ValidItemTypes()
    {
      ...
      foreach (KeyValuePair<string, Dictionary<string, object>> entry in doDictionary[cITEMTYPE])
        itemTypeList.Add(entry.Key);
      return itemTypeList;
    }
    /// <summary>
    ...
    public object DictionaryProperty(string dictionaryType, string dictionaryItem, string property)
    {
      try
      {
        if (doDictionary.ContainsKey(dictionaryType)
            && doDictionary[dictionaryType].ContainsKey(dictionaryItem)
            && doDictionary[dictionaryType][dictionaryItem].ContainsKey(property))
          return doDictionary[dictionaryType][dictionaryItem][property];
        return string.Empty;
      }
      catch
      {
    ...
    public bool IsValid(Item item, string attributeName, out string errorMessage)
    {
      ...
      if (item.Attributes.Contains(attributeName))      // is this attribute in the collection?
      {
        ...
        if (isValid)                      // only perform the next check if the last one passed
        {
          ...
          {
            PickList pickList = ((TableHeader)TAGFunctions.evaluateFunction(TAGFunctions.EnumFunctions.CTableHeader, pickListName, item.Attributes[pickListName].Value)).ToPickList();     // get the pick list string
            isValid = passesPickList(pickList, pValue);    // is in the the pick list?
            if (!isValid)
              errorMessage = pValue.ToString() + " is not in the list of valid values"; // nope, set the error message
          }
    ...
    private List<string> checkToken(string token, string returnType, List<string> msgList)
    {
      ...
      string strExpression = (string)TAGFunctions.evaluateFunction(TAGFunctions.EnumFunctions.CString, TAGFunctions.evaluateFunction(TAGFunctions.EnumFunctions.stripComment, token, true));
      ...
      switch (strExpression.Substring(0, 1))
      {
        ...
        case FUNC:
          ...
          {
            object oParms = DictionaryProperty(cFUNCTION, functionName, "Parameters");
            if (!((bool)TAGFunctions.evaluateFunction(TAGFunctions.EnumFunctions.IsTableHeader, oParms)))
              msgList.Add(string.Format("{0}({1}): Function Name is not registered", functionName, insideExpression));
            else
            {
              TableHeader parameters = (TableHeader)TAGFunctions.evaluateFunction(TAGFunctions.EnumFunctions.CTableHeader, oParms);
                  ...
                  for (int i = 0; i < parameters.Count; i++)
                  {
                    if (((string)TAGFunctions.evaluateFunction(TAGFunctions.EnumFunctions.CString, parameters[i, "Parameter"])).Equals("ReturnType", StringComparison.CurrentCultureIgnoreCase))
                    {
                      ...
                      if (!isCompatibleDataType(funcDataType, returnType))
                        msgList.Add(string.Format("Function: {0}: Return data type {1} must be the same as receiving data type {2}",
                          functionName, funcDataType, returnType));
                      break;
                    }
                  }
                }
    ...
    private bool passesValidation(string validationString, string valueType, object pValue, out string pErrorMessage, out bool warn)
    {
      ...
      for (; i < validationTable.GetLength(0) && isValid; i++) // for each row or until a validation fails
      {
        ...
        //TODO: REF Parameter: We need to review this!
    ...
    private void load(EntityAttributesCollection eac)
    {
      if (eac.Entities.Contains(cDICTIONARY))
      {
        foreach (ItemType it in eac.Entities[cDICTIONARY].ItemTypes)
        {
          Dictionary<string, Dictionary<string, object>> doIT = new Dictionary<string, Dictionary<string, object>>(StringComparer.CurrentCultureIgnoreCase);
          foreach (Item i in it.Items)
          {
            Dictionary<string, object> doItem = new Dictionary<string, object>(StringComparer.CurrentCultureIgnoreCase);
            foreach (TAGAttribute a in i.Attributes)
              doItem.Add(a.OriginalID, a.Value);
            doIT.Add(i.OriginalID, doItem);
          }
          doDictionary.Add(it.OriginalID, doIT);
        }
      }
    }
