      ...
      else
      {
        ...
        else
        {
          ...
          if (lblDirectCharge.Visible) // this is an expense
            fields.Add(CommonData.fieldBILLEDAMOUNT, CommonFunctions.CDecimal(txtDirectCharge.Text));
          else
          {
    ...
    private void loadTime(int id)
    {
      ...
      if (timeEntry != null && timeEntry.NumberRows > 0)
      {
        ...
        if (_resource.Equals("Expenses", StringComparison.CurrentCultureIgnoreCase))
        {
          ...
          txtDirectCharge.Text = CommonFunctions.CString(timeEntry[0, CommonData.fieldBILLEDAMOUNT]);
        }
        else
        {
