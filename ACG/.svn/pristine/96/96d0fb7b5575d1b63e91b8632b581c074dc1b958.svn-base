...
namespace ACG.CommonForms
{
  public partial class ctlDayPlanner : UserControl
  {
    ...
    private TimeSpan _increment = new TimeSpan(0,15,0);
    private DateTime _beginTime;
    private DateTime _displayBeginTime;
    private DateTime _endTime;
    private KnownColor _onHourColor = KnownColor.LightGray;
    private KnownColor _alternateColor = KnownColor.LightGray;
    ...
    public void LoadData(List<TimeEntry> timeEntries)
    {
      ...
      foreach (TimeEntry te in _timeEntries)
      {
        ...
        int incrementsPerHours = 60 / minutes;
        TimeSpan diffTime = _endTime - _beginTime;
        int totalSlots = Convert.ToInt32(diffTime.TotalHours * Convert.ToDouble(incrementsPerHours)); // total number of slots we have
        TimeSpan startTime = roundTime(te.StartTime).TimeOfDay;
        int slotsFromMidnight = (startTime.Hours * incrementsPerHours) + (startTime.Minutes / minutes);
        int offset = (_beginTime.Hour * incrementsPerHours) + (_beginTime.Minute / minutes);
        ...
        for (int i = 0; i < nbrSlots; i++)
        {
          ...
          if (panelMain.Controls.ContainsKey(ctlName))
          {
            ...
            {
              if (te.Descriptions.GetLength(0) == 1)
              {
                if (!string.IsNullOrEmpty(te.Descriptions[1]))
                  ctl.Text = te.Descriptions[1];
              }
              else
              {
                if (!string.IsNullOrEmpty(te.Descriptions[i]))
...
//      int totalSlots = (_endTime - _beginTime).Days * 48 + (_endTime - _beginTime).Minutes / minutes; // total number of slots we have
      int totalSlots = (_endTime - _beginTime).Days * 24 * (60 / minutes); // total number of slots we have
      DateTime timeTitle = _beginTime;
      bool onHour = true;
