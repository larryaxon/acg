...
namespace ACG.Common
{
  ...
  public class Security 
  {
    ...
    protected const string cGRANT = "Grant";
    protected const string cDENY = "Deny";
    protected const string cNONE = "None";          // special right that is worse than any right, including Deny
    protected const string cREADONLY = "ReadOnly";
    ...
    protected const string EM_SYSTEMOBJECT_LOOKUP_FAILED = "Lookup of SystemObject {0} failed"; //this string is formated depending the system object failed
    protected const string EM_ENTITY_LOOKUP_FAILED = "Lookup of Entity {0} failed";
    #endregion constants

    public Security() { }
    /// <summary>
    /// Creates a new Security instance from the custom serialized string created by ToString()
    /// </summary>
    /// <param name="securityString"></param>
    public Security(string securityString)
    {
      if (string.IsNullOrEmpty(securityString))
        throw new Exception("Security constructor cannot accept empty string");
      StringBuilder sb = new StringBuilder(securityString);
      if (!securityString.StartsWith("Security:(User:") && !securityString.StartsWith("\"user\":") && !securityString.StartsWith("(User:"))
        throw new Exception("Security constructor: invalid security string");
      AccessRightNames = CommonData.AccessRightNames;
      string s = CommonFunctions.stripDelims(securityString, '(');
      string[] tokens = CommonFunctions.parseString(s, new string[] { "," });
      foreach (string token in tokens)
      {
        string[] entry = CommonFunctions.parseString(token, new string[] { ":" });
        if (entry.GetLength(0) != 2)
          throw new Exception(string.Format("Security constructor: invalid token={0}", token));
        string key = CommonFunctions.stripDelims(entry[0].ToLower(),CommonData.cDOUBLEQUOTE);
        ...
        switch (key)
        {
          case "user":
            ...
            break;
          case "isloggedin":
            isLoggedIn = CommonFunctions.CBoolean(val);
            break;
          case "parentgroups":
            string[] parents = CommonFunctions.parseString(CommonFunctions.stripDelims(entry[1],CommonData.cLEFTSQUARE));
            foreach (string parent in parents)
              ParentGroups.Add(CommonFunctions.stripDelims(parent, CommonData.cDOUBLEQUOTE));
            break;
          case "objectrightslist":
            loadDictionary(ObjectRightsList, CommonFunctions.stripDelims(entry[1], CommonData.cLEFTSQUARE), true);
            break;
          case "objecttypes":
            loadDictionary(ObjectTypes, CommonFunctions.stripDelims(entry[1], CommonData.cLEFTSQUARE), true);
            break;
          default:
            throw new Exception(string.Format("Security constructor: invalid token={0}", token));
        }
    ...
    private void loadDictionary(Dictionary<string, string> dict, string source, bool allowEmptyToken)
    {
      /*
       * TODO: Review this logic! Since it will report an error even if the group is empty, wich could be a valid situation
       * Need to talk with Larry about this problem for now I will ACCEPT some EMPTY tokens!
       */
      if (dict == null || (string.IsNullOrEmpty(source) && !allowEmptyToken))
        throw new Exception(string.Format("Security constructor: invalid token={0}", source));
      string tokensString;
      /*
       * This was added by LLA on 10/19/2012
       * 
       * If this is a list of json-style objects, as in {Name: val}, {name:val}, then the stripdelim was taking off the first and last 
       * curly braces, and it was confusing the paren depth and the parser. So I added this as a special case to bypass the stripdelims
       * in this case, and also to stip off the delims of each token, once they are split up, so the middle delim (:) works.
       * 
       */
      bool isJson = source.Contains("},{") && !source.StartsWith("{{");
      if (isJson) // this means this is a list in the form {name:val},{name:val}...
        tokensString = source;
      else
        tokensString = CommonFunctions.stripDelims(source, CommonData.cLEFTCURLY);
      string[] tokens = CommonFunctions.parseString(CommonFunctions.stripParens(tokensString), new string[] { "," });
      foreach (string token in tokens)
      {
        string t = token;
        if (isJson)
          t = CommonFunctions.stripDelims(token, CommonData.cLEFTCURLY);
        string[] entry = CommonFunctions.parseString(t, new string[] { ":" });
        dict.Add(CommonFunctions.stripDelims(entry[0],CommonData.cDOUBLEQUOTE), CommonFunctions.stripDelims(entry[1], CommonData.cDOUBLEQUOTE));
      }
    }
    #region module data
    ...
    private ArrayList accessRightNames = new ArrayList();               // populated by LoadPrecedence
    /*
    ...
    ArrayList myParents = new ArrayList();                                // list of groups of which I am a member
 ...
 * 
 */
    private Dictionary<string, string> objectRightsList = new Dictionary<string, string>(StringComparer.CurrentCultureIgnoreCase);      // hash lookup of rights by object
    private Dictionary<string, string> entityRightsList = new Dictionary<string, string>(StringComparer.CurrentCultureIgnoreCase);      // hash lookup of rightys by entity
    private Dictionary<string, string> objectTypes = new Dictionary<string, string>(StringComparer.CurrentCultureIgnoreCase);
    ...
    {
      StringBuilder sbParentGroups = new StringBuilder();
      sbParentGroups.Append("(");
      if (ParentGroups != null)
      {
        foreach (string parent in ParentGroups)
        {
          sbParentGroups.Append(parent);
          sbParentGroups.Append(",");
        }
        sbParentGroups.Length = sbParentGroups.Length - 1;  // strip off last comma
      }
      sbParentGroups.Append(")");
      StringBuilder sbObjectRights = new StringBuilder();
      sbObjectRights.Append("(");
      foreach (KeyValuePair<string, string> right in ObjectRightsList)
        sbObjectRights.Append(string.Format("{0}:{1},", right.Key, right.Value));
      sbObjectRights.Length = sbObjectRights.Length - 1; // strip off last comma
      sbObjectRights.Append(")");
      StringBuilder sbObjectTypes = new StringBuilder();
      sbObjectTypes.Append("(");
      foreach (KeyValuePair<string, string> type in ObjectTypes)
        sbObjectTypes.Append(string.Format("{0}:{1},", type.Key, type.Value));
      sbObjectTypes.Length = sbObjectTypes.Length - 1; // strip off last comma
      sbObjectTypes.Append(")");
      return string.Format("Security:(User:{0},IsLoggedIn:{1},ParentGroups:{2},ObjectRightsList:{3},ObjectTypes:{4})",
        User, IsLoggedIn.ToString(), sbParentGroups.ToString(), sbObjectRights.ToString(), sbObjectTypes.ToString());
    }
    ...
    public string ToJson()
    {
      ...
      if(sbParentGroups[sbParentGroups.Length-1] == ',')
        sbParentGroups.Length = sbParentGroups.Length - 1;  // strip off last comma
      sbParentGroups.Append("]");
      StringBuilder sbObjectRights = new StringBuilder();
      sbObjectRights.Append("[");
      foreach (KeyValuePair<string, string> right in ObjectRightsList)
        sbObjectRights.Append(string.Format("{{\"{0}\":\"{1}\"}},", right.Key, right.Value));
      if (sbObjectRights[sbObjectRights.Length - 1] == ',')
        sbObjectRights.Length = sbObjectRights.Length - 1; // strip off last comma
      sbObjectRights.Append("]");
      StringBuilder sbObjectTypes = new StringBuilder();
      sbObjectTypes.Append("[");
      foreach (KeyValuePair<string, string> type in ObjectTypes)
        sbObjectTypes.Append(string.Format("{{\"{0}\":\"{1}\"}},", type.Key, type.Value));
      if (sbObjectTypes[sbObjectTypes.Length - 1] == ',')
        sbObjectTypes.Length = sbObjectTypes.Length - 1; // strip off last comma
      sbObjectTypes.Append("]");
      return string.Format("\"security\":{{\"user\":\"{0}\",\"isloggedIn\":{1},\"ParentGroups\":{2},\"ObjectRightsList\":{3},\"ObjectTypes\":{4}}}",
        User, IsLoggedIn.ToString().ToLower(), sbParentGroups.ToString(), sbObjectRights.ToString(), sbObjectTypes.ToString());
    }
