...
namespace CCI.Common
{
  ...
  public class SortedCollectionBase : IEnumerable
  {
    ...
    public ISortedItemBase this[string key]
    {
      ...
      {
        if (key == null)
          throw new Exception("Null ID is not valid");
        string k = key.ToLower();
        if (!Contains(k))
          throw new KeyNotFoundException();
        else
          return (ISortedItemBase)HashTable[k];
      }
      set
      {
        if (key == null)
        ...
        {
          HashTable[k] = value;
          int pos = binarySort(this.List, value);
          if (pos == -1) // this should never happen
            throw new Exception("Hashtable and List are out of sync");
          List[pos] = value;
        }
        else
          Add(value);
    ...
    public ISortedItemBase this[int index]
    {
      get
      {
        if (index < 0 || index >= Count)
          ...
          return (SortedItemBase)List[index];
      }
      set
      {
        if (index < 0 || index >= Count)
        ...
        else
        {
          ...
          ISortedItemBase i = (SortedItemBase)List[index];  // get old value in that index
          if (!value.ID.Equals(i.ID, StringComparison.CurrentCultureIgnoreCase))  // if the IDs are not the same
          {
            HashTable.Remove(i.ID); // then get rid of the old item in the hash table and add the new one
            HashTable.Add(value.ID, value);
          }
          else
            HashTable[value.ID] = value;  // otherwise just replace it
          List[index] = value;
        }
      }
    ...
    {
        int iSort = CommonFunctions.CInt(sortOrder, 0);
