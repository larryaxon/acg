...
namespace CCI.Sys.Data
{
  ...
  public class EntityDB : DataAccessBase
  {
    ...
    public void UpdateRecord(Entity entity)
    {
      ...
      foreach (Field f in entity.Fields)
      {
        if (!f.Virtual)
        {
          ...
          if (f.OriginalID.Equals("EndDate",StringComparison.CurrentCultureIgnoreCase) || f.OriginalID.Equals("StartDate",StringComparison.CurrentCultureIgnoreCase))
            values[i] = ((DateTime)TAGFunctions.evaluateFunction(TAGFunctions.EnumFunctions.CDateTime,f.Value)).ToShortDateString();
          else
    ...
    public DataSet searchEntities(string criteria, DateTime effectiveDate, string entityType, string entityOwner, bool includeTermed)
    {
      ...
      criteria = CCI.Common.CommonFunctions.fixUpStringForSQL(criteria);
