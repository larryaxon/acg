...
namespace CCI.Sys.Data
{
  public partial class DataSource
  {
    ...
    public string getNextLocationID(string entityowner)
    {
      /*
       *  Location format is 1XXXXSSSS
       *  where XXXX is the last 4 digits of the entityowner and SSSS is a uniue number for the location
       */
      if (string.IsNullOrEmpty(entityowner) || entityowner.Length != 5)
        return null;
      string sql = string.Format("select max(entity) Entity from entity where entityowner = '{0}' and entitytype = 'location'", entityowner);
      using (DataSet ds = getDataFromSQL(sql))
      {
        string lastLocationID = string.Format("1{0}0001",entityowner.Substring(1)); //fake one starting at one in case there are none yet
        if (ds == null)
          return lastLocationID;
        if (ds.Tables.Count == 0 || ds.Tables[0].Rows.Count == 0)

          return lastLocationID;
        if (ds.Tables[0].Rows[0]["Entity"] == null)
          return lastLocationID;
        string tmploc = CommonFunctions.CString(ds.Tables[0].Rows[0]["Entity"]);
        if (string.IsNullOrEmpty(tmploc))
          return lastLocationID;
        lastLocationID = tmploc;
        string lastLocationSeq = lastLocationID.Substring(5);
        if (CommonFunctions.IsNumeric(lastLocationSeq))
        {
          int nextseq = Convert.ToInt32(lastLocationSeq) + 1; // get the next available
          return string.Format("1{0}{1}", entityowner.Substring(1), nextseq.ToString("0000"));
        }
        else
          return null; ;
      }
