...
namespace CCI.Sys.Data
{
  public partial class DataSource : DataAccessBase
  {
    ...
    public void addCodeMasterCode(string codeType, string code, string user)
    {
      string sql = string.Format("Select count(*) from CodeMaster where CodeType = '{0}' and CodeValue = '{1}'", codeType, code);
      bool hasCode = false;
      using (DataSet ds = getDataFromSQL(sql))
      {
        if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
          if (CommonFunctions.CInt(ds.Tables[0].Rows[0][0]) > 0)
            hasCode = true;
      }
      if (!hasCode)
      {
        sql = string.Format(@"
Insert Into CodeMaster (CodeType, CodeValue, Description, LastModifiedBy, LastModifiedDateTime)
Values ('{0}','{1}','{1} Pricing Level','{2}','{3}')"
,codeType, code, user, DateTime.Now.ToString(CommonData.FORMATLONGDATETIME));
        updateDataFromSQL(sql);
      }
    }
    public void deleteMasterCode(string codeType, string code)
    {
      string sql = string.Format("Select count(*) from CodeMaster where CodeType = '{0}' and CodeValue = '{1}'", codeType, code);
      bool hasCode = false;
      using (DataSet ds = getDataFromSQL(sql))
      ...
      if (hasCode)
      {
        sql = string.Format(@"DELETE FROM CodeMaster WHERE CodeType = '{0}' and CodeValue = '{1}'", codeType, code);
        updateDataFromSQL(sql);
      }
    }
