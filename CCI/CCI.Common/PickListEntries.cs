...
{
  [Serializable]
  public class PickListEntries : IEnumerable, IEquatable<PickListEntries>, IComparable
  {
    ...
    public bool SortedByAscending = true;
    ...
    public SortedDictionary<string, PickListEntry> SortedBy(string memberName)
    {
      DescendingComparer t = new DescendingComparer();
      SortedDictionary<string, PickListEntry> list;
      if (SortedByAscending)
        list = new SortedDictionary<string, PickListEntry>(StringComparer.CurrentCultureIgnoreCase);
      else
        list = new SortedDictionary<string, PickListEntry>(t);
      string key = string.Empty;
      foreach (PickListEntry s in this)
      {
    ...
    public int CompareTo(object o)
    {
      if (o == null)
        return 1;
      if (o.GetType() != typeof(PickListEntries))
        throw new Exception("Must CompareTo type PickListEntries");
      return CompareTo((PickListEntries)o);

    }
    public int CompareTo(PickListEntries list)
    {
      if (list == null)
        return 1;
      for (int i = 0; i < Count; i++)
      {
        int cmp = this[i].CompareTo(list[i]);
        if (cmp != 0)
          return cmp;
      }
      return 0;
    }
    public bool Equals(PickListEntries list)
    {
